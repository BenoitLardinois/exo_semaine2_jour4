...
puts handles.min_by(&:length)

#3
puts "Cmb y-a-t'il de handle contenant 5 caractères ?"
puts handles.count {|item|6 == item.length}

#4
puts "Cmb commencent par un majuscule ?"
capital_letters = ("A".."Z")
puts handles.count {|x| capital_letters.include?(x[1])}

#5
puts "Trier la liste de handle par ordre alphabétique"
puts handles.sort_by(&:downcase)

#6
puts "Trier la liste de handle par taille des handles (du plus petits aux plus grands)"
puts handles.sort_by(&:length)

#7
puts "Quelle est la position dans l'array de la personne @epenser ?"
puts handles.index('@epenser')
